...
DROP FUNCTION IF EXISTS public.send_feedback_email(text, text, text, text);

-- Criar extensão para gerar UUIDs se ainda não existir
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS http;

-- Criar função para enviar email de feedback usando a API do Resend
...
BEGIN
  ...
  -- Obter API key do Resend da tabela de configurações
  SELECT get_config('resend_api_key') INTO v_api_key;
  ...
  v_debug_info := v_debug_info || jsonb_build_object(
    ...
    'api_key_preview', 're_***' || right(v_api_key, 4)
  );
