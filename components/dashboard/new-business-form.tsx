    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center">
        <Avatar className="h-20 w-20">
          <AvatarImage src={logoPreview || "/placeholder.svg"} alt="Logo" />
          <AvatarFallback className="text-2xl">{getBusinessInitials(form.watch("name"))}</AvatarFallback>
        </Avatar>
        <div>
        ...
        <Button type="submit" disabled={isLoading || isUploading}>
          {isLoading || isUploading ? (
            <>
              ...
              {isUploading ? "Enviando logo..." : "Criando negócio..."}
            </>
          ) : (
            "Criar negócio"
          )}
